...
setup_dart_dependencies() {
    print_step "Setting up Dart dependencies for report generation..."
    local original_dir=$(pwd)
    cd "$SCRIPT_DIR/dart"
    dart pub get
    cd "$original_dir"
...
run_tests() {
    ...
    for test_file in "${ARBIGENT_PROJECTS[@]}"; do
        ...
        if [ -d "arbigent-result" ]; then
            ...
            local original_dir=$(pwd)
            local result_path="$original_dir/arbigent-result"
            cd "$SCRIPT_DIR/dart"
            dart run generate_report.dart --path="$result_path"
            cd "$original_dir"
...
show_help() {
    echo "QuickPizza Flutter E2E Test Runner"
    echo ""
...
main() {
    ...
    print_step "HTML reports available in: $FLUTTER_ROOT/arbigent-result/"
}
