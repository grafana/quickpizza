...
class ApiService {
  ...
  Future<PizzaRecommendation?> getPizzaRecommendation(
    ...
    try {
      final response = await http.post(
        Uri.parse('$baseUrl/api/pizza'),
        headers: _headers,
        body: jsonEncode(restrictions.toJson()),
      );
      if (response.statusCode == 200) {
        final json = jsonDecode(response.body) as Map<String, dynamic>;
  ...
  Future<bool> ratePizza(int pizzaId, int stars) async {
    try {
      final rating = Rating(id: 0, pizzaId: pizzaId, stars: stars, userId: 0);
      final response = await http.post(
        Uri.parse('$baseUrl/api/ratings'),
        headers: _headers,
        body: jsonEncode(rating.toJson()),
      );
      // 201 = created, 200 = OK, 401 = authentication required
