...
| HTTP client (recommendations → catalog/copy, and the gateway's reverse proxy) | `otelhttp.NewTransport` (auto) | Spans, metrics | A client span for every outbound HTTP call one internal service makes to another (e.g. recommendations calling catalog for ingredients). These spans are children of the request span that triggered the call, so a single pizza request produces a full call tree across services. Also emits `http_client_request_duration` metrics. In `obi` mode (`cmd/main.go`'s `newRecommendationsHTTPClient` and `pkg/http/http.go`'s gateway proxy transport) this wrapping is skipped entirely, since OBI captures this traffic via eBPF and app-side `otelhttp` would just duplicate it — see "Two instrumentation modes" above. |
