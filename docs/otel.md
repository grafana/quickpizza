...
| Application counters/histograms | Prometheus client (not OTel) | Metrics | Domain-specific metrics that OTel has no concept of, e.g. how many pizzas were recommended, how many ingredients/calories a pizza had, split by vegetarian/tool. These are a parallel, separate metrics pipeline from OTel's HTTP metrics, exposed on `/metrics`. No spans, no OTel involvement at all. |
